...
public class DataMiner {
    ...
    public void run() throws Exception {
        ...
        methods.alert(max + "", "max");

        int hospitalNumber = hospitalResults.indexOf(max);

        String mostVolumeHospital = hospitals.get(hospitalNumber);

        methods.alert("The hospital with the most " + getTypeByColumn(type) + " volume is " + mostVolumeHospital.toLowerCase() + ".", "Results");
    }
    ...
    private int requestType() {
        ...
        int okcancel = JOptionPane.showConfirmDialog(null, question, "Settings", JOptionPane.OK_CANCEL_OPTION);

        int type = layoutAnswer.getSelectedIndex();

        type = methods.getColumnByType(type);
